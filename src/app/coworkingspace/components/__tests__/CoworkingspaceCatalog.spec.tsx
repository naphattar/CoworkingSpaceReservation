...
import CoworkingspaceCatalog from "../CoworkingspaceCatalog";
...
describe("CoworkingspaceCatalog Component", () => {
  ...
  it("renders the correct number of coworking spaces", () => {
    ...
    const coworkingCards = screen.getAllByRole("link"); // Links wrapping the Card component
  ...
  it("links to the correct coworking space pages", () => {
    ...
    mockCoworkingspaces.forEach((space) => {
      const link = screen.getByRole("link", { name: `${space.name} ${space.name}`});
